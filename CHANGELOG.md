...
- **Kit-level aspect registry retired; CI gates now read pyproject.toml + per-aspect manifests** (plan `retire-kit-aspects-yaml`, ADR-0040 §7 operational completion). The substrate's CI gates (`scripts/check_kit_consistency.py`, `scripts/check_package_contents.py`, the drift sentinel in `tests/test_aspect_provides.py`) no longer read `packages/kanon-core/src/kanon_core/kit/manifest.yaml` (the file is deleted). The new oracle is `pyproject.toml`'s `[project.entry-points."kanon.aspects"]` table at the workspace root, plus each aspect's per-aspect `manifest.yaml` (canonical per ADR-0055). Closes a long-standing publisher-symmetry violation: the kit YAML was a `kanon-`-only artifact no `acme-` publisher could replicate; the new oracle path is shape-equivalent across publishers.

### Added

- **Publisher-symmetry CI gate (per `P-publisher-symmetry §Implications`).** `tests/scripts/test_publisher_symmetry.py` runs on every PR + merge-to-main against the synthetic `tests/fixtures/acme-test-aspects/` overlay (an `acme-test`-namespaced bundle). Asserts `scripts/check_kit_consistency.py`'s algorithm is publisher-blind: zero structural errors against the acme overlay, just like the kit's own bundle. Future regressions that accidentally hardcode `kanon-` privilege turn this step red.
- **`docs/reference-aspects.md` — generated discoverability artifact.** Publisher-neutral roster of the 7 reference aspects (stability / depth-range / default-depth / description). Generated by `scripts/gen_reference_aspects.py` from `pyproject.toml`'s entry-points + per-aspect manifests; CI gate `gen_reference_aspects.py --check` enforces freshness on every commit.
