...
- **Recipe authorship grammar.** Recipes are publisher-shipped target-tree YAML. The substrate must accept any conforming recipe regardless of which publisher authored it — `acme-fintech-default` and `kanon-aspects-default` resolve through the same code path.
...
- A consumer running `pip install acme-<name>-<aspect>` and `kanon aspect add . acme-<name>-<aspect>` gets a working configuration with no kernel-side privilege over what `kanon-aspects` aspects would have provided.
