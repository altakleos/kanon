...
- **Self-hosting falsification probes are the substrate's primary correctness signal.** P1 ("the kanon repo cannot self-host on the latest substrate from a fresh clone with explicit opt-in") and P2 ("two `kanon verify` runs on the same SHA produce non-byte-equal JSON") fire on the substrate's own evidence with no LLM, no consumers, no Tier-2 infrastructure. P0 ("two clean LLM resolutions disagree on the same `(contract, model, inputs)`") tests the resolver layer.
