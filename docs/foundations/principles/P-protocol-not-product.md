...
- **Reference aspects ship in a separate distribution.** `kanon-core` (kernel) and `kanon-aspects` (the seven `kanon-` aspects as data) are independently installable; `kanon-kit` is a meta-alias for the convenience-install path. A consumer running only `kanon-core` gets a working substrate with zero opinions about discipline.
- **The substrate's test suite must pass with `kanon-aspects` uninstalled.** Self-host is achieved by *the kanon repo opting into a publisher recipe*, not by the kernel auto-knowing about reference aspects.
