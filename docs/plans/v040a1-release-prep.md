...
4. `_aspect_path()` legacy fallback fix — `src/kanon/_manifest.py:721`: when `kanon_reference` import fails AND aspect is `kanon-*`, raise `click.ClickException("kanon-substrate cannot serve kanon-* aspects without kanon-reference installed; install kanon-kit or kanon-reference")` instead of returning a dead `_kit_root() / aspects / <slug>` path. Test: mask `kanon_reference`, call `_aspect_path("kanon-sdd")`, assert exception with helpful message.
