...
- [ ] AC-A1: `docs/decisions/0040-kernel-reference-runtime-interface.md` exists with `status: accepted` and the six required ADR sections.
- [ ] AC-A2: ADR-0040 cites ADR-0048 (parent), ADR-0028 (project-aspect grammar), ADR-0026 (capability registry), ADR-0039 (resolution model).
- [ ] AC-A3: At least five Alternatives Considered, each with clear rejection rationale.

### Design

- [ ] AC-D1: `docs/design/kernel-reference-interface.md` contains the `[project.entry-points."kanon.aspects"]` shape with worked example.
- [ ] AC-D2: Registry composition algorithm specified with enough detail that Phase A can implement without further design work.
- [ ] AC-D3: Independence invariant test design specified concretely (CI gate algorithm).
- [ ] AC-D4: Every call site of `_kit_root()` walked with replacement specified.

### Spec amendments

- [ ] AC-S1: `docs/specs/aspects.md` gains a one-paragraph "Protocol-substrate composition" section citing ADR-0040; existing INVs unchanged.
- [ ] AC-S2: `docs/specs/project-aspects.md` gains a parallel section.

### Indexes + CHANGELOG

- [ ] AC-X1: `docs/decisions/README.md` updated with ADR-0040 row.
- [ ] AC-X2: `docs/design/README.md` updated with `kernel-reference-interface.md` row.
- [ ] AC-X3: `CHANGELOG.md` `## [Unreleased] § Added` gains a paragraph naming ADR-0040.
