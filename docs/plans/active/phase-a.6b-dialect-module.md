...
- [ ] AC-M1: `src/kanon/_dialects.py` exists with `SUPPORTED_DIALECTS`, `DEPRECATION_WARNING_BEFORE`, `validate_dialect_pin` symbols.
- [ ] AC-M2: `SUPPORTED_DIALECTS` contains `"2026-05-01"` (the ratified v1 dialect per ADR-0041).
- [ ] AC-M3: `DEPRECATION_WARNING_BEFORE` is empty (no current deprecations).
- [ ] AC-M4: `validate_dialect_pin("2026-05-01")` succeeds (no exception, no warning).
- [ ] AC-M5: `validate_dialect_pin("unknown")` raises `click.ClickException` with text including the supported list.
- [ ] AC-M6: `validate_dialect_pin(None)` raises `click.ClickException` ("missing pin").
- [ ] AC-M7: When `DEPRECATION_WARNING_BEFORE` contains the pin (test via monkeypatch), a stderr warning fires.

### Tests

- [ ] AC-T1: `tests/test_dialects.py` exercises ≥10 cases covering the 6 invariants.
- [ ] AC-T2: Full pytest passes.

### CHANGELOG

- [ ] AC-X1: `CHANGELOG.md` `[Unreleased] § Added` gains a paragraph naming Phase A.6b.
...
- [ ] AC-X9: No CLI verb additions.
- [ ] AC-X10: No `_manifest.py` integration (deferred).
