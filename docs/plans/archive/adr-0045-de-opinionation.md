...
- **The transition sequence** — concrete ordered list:
  ...
  - **Phase A.8**: All four scaffolded `scripts/check_*.py` files retired from `kanon-aspects`'s scaffolded files; consumer-side equivalents remain in the kanon repo as authored realizations (per the protocol-substrate's "kit teaches; consumer realizes; agent binds" model).
...
- **Consequences** — what changes for Phase A's PR sequencing; for the substrate-independence gate's expected first-run-red status; for `kanon-core==1.0.0a1` shipping as a hard cut.
...
- [x] AC-A1: `docs/decisions/0045-de-opinionation-transition.md` exists with `status: accepted`.
- [x] AC-A2: ADR-0045 cites ADR-0048, ADR-0040, ADR-0043, ADR-0044, the de-opinionation manifesto.
- [x] AC-A3: At least four Alternatives Considered.
- [x] AC-A4: Concrete Phase 0.5 + 9-step Phase A sequence in the Decision section.

### Indexes + CHANGELOG

- [x] AC-X1: `docs/decisions/README.md` updated with ADR-0045 row.
- [x] AC-X2: `CHANGELOG.md` `[Unreleased] §Added` gains a paragraph naming ADR-0045.
- [x] AC-X3: CHANGELOG `[Unreleased]` notes Phase 0 closure (all seven ADRs ratified).

### Cross-cutting

- [x] AC-X4: `kanon verify .` → `status: ok`, zero warnings.
- [x] AC-X5: `python scripts/check_links.py` → ok.
- [x] AC-X6: `python scripts/check_foundations.py` → ok.
- [x] AC-X7: `python scripts/check_invariant_ids.py` → ok.
- [x] AC-X8: No source / aspect-manifest / protocol-prose / CI / new-spec / new-design changes.
