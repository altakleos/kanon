...
- [x] AC-M1: `_resolutions.py` exposes `_validate_shape_against_contract` and calls it from `_replay_inner` after pin-check
- [x] AC-M2: Skip-when-absent: contracts without `realization-shape:` produce no shape findings
- [x] AC-M3: Shape violations surface as `ReplayError` with the appropriate code
- [x] AC-T1: ≥5 new tests passing
