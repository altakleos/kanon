...
Per [ADR-0040](../../decisions/0040-kernel-reference-runtime-interface.md), [ADR-0044](../../decisions/0044-substrate-self-conformance.md), [ADR-0048](../../decisions/0048-kanon-as-protocol-substrate.md). The substrate-independence invariant requires `kanon-core` to ship NO aspect data; aspects must travel with `kanon_reference`. Today, the seven kanon-* aspects' protocols / files / sections / agents-md still live at `src/kanon/kit/aspects/<X>/`. This plan moves them under `src/kanon_reference/data/<X>/`.
...
- [x] AC-M1: All 42 files moved from `src/kanon/kit/aspects/<X>/...` to `src/kanon_reference/data/<X>/...`
- [x] AC-M2: `src/kanon/kit/aspects/` is empty (or contains only `manifest.yaml` for kit-globals if any)
- [x] AC-M3: Substrate's `_load_aspects_from_entry_points()` synthesizes `_source` pointing at new data location
- [x] AC-M4: `tests/test_kanon_reference_manifests.py` reads from new location
- [x] AC-M5: Fidelity lock recaptured
- [x] AC-M6: `kanon verify .` ok; full pytest passes
- [x] AC-X1..X8: standard gates green
