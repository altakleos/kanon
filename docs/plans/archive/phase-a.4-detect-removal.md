...
- [x] AC-D1: `src/kanon/_detect.py` no longer exists.
- [x] AC-D2: `tests/test_detect.py` no longer exists.

### Substrate code

- [x] AC-S1: No source file imports `kanon._detect`.
- [x] AC-S2: `cli.py` `kanon init` no longer auto-fills `aspects_meta["kanon-testing"]["config"]`.
- [x] AC-S3: `cli.py:_emit_init_hints()` no longer emits "Preflight readiness" section (function body trimmed or function deleted entirely).

### LOADER MANIFEST + YAML

- [x] AC-M1: `src/kanon_reference/aspects/kanon_testing.py:MANIFEST` no longer contains a `config-schema` key.
- [x] AC-M2: `src/kanon/kit/aspects/kanon-testing/manifest.yaml` no longer contains a `config-schema:` block.
- [x] AC-M3: `.kanon/recipes/reference-default.yaml`'s kanon-testing entry no longer carries a `config:` block.
- [x] AC-M4: `.kanon/config.yaml`'s `aspects.kanon-testing.config` block deleted (or empty `{}`).

### Tests

- [x] AC-T1: `tests/test_kanon_reference_manifests.py` parametrized tests pass (LOADER MANIFEST union with top-entry equals expected — config-schema removed from both sources, equivalence still holds).
- [x] AC-T2: Audit completed; tests that asserted preflight-readiness output or testing config-schema updated/deleted.
- [x] AC-T3: Full pytest passes.
...
- [x] AC-X1: `CHANGELOG.md` `[Unreleased] § Removed` gains a paragraph naming Phase A.4.

### Cross-cutting

- [x] AC-X2: `kanon verify .` returns `status: ok`, zero warnings.
- [x] AC-X3..X6: standard gates pass (`check_links`, `check_foundations`, `check_kit_consistency`, `check_invariant_ids`, `check_packaging_split`).
- [x] AC-X7: No `src/kanon_reference/` change beyond the kanon-testing config-schema deletion.
- [x] AC-X8: No aspect content moved.
- [x] AC-X9: No `_kit_root()` call sites in `_scaffold.py` retired (those are deferred to content-move sub-plan).
