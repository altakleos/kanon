...
- [x] AC-R1: `.kanon/recipes/reference-default.yaml` exists with the schema per ADR-0043's design.
- [x] AC-R2: Recipe lists the seven reference aspects at the same depths the current `.kanon/config.yaml` declares (kanon-sdd:3, kanon-testing:3, kanon-worktrees:2, kanon-release:2, kanon-security:2, kanon-deps:2, kanon-fidelity:1).
- [x] AC-R3: Recipe's `target-dialect` is `2026-05-01`.

### Config augmentation

- [x] AC-C1: `.kanon/config.yaml` has new top-level fields `schema-version: 4`, `kanon-dialect: "2026-05-01"`, `provenance:`.
- [x] AC-C2: `.kanon/config.yaml`'s existing v3 fields (`kit_version`, `aspects:`, `preflight-stages:`) are preserved verbatim — no field removed, no value changed.

### CHANGELOG

- [x] AC-X1: `CHANGELOG.md` `[Unreleased] § Changed` gains a paragraph naming Phase 0.5.

### Cross-cutting

- [x] AC-X2: `kanon verify .` returns `status: ok`, zero warnings (the current kit ignores v4 fields).
- [x] AC-X3: `python scripts/check_links.py` passes.
- [x] AC-X4: `python scripts/check_foundations.py` passes, zero warnings.
- [x] AC-X5: `python scripts/check_kit_consistency.py` passes (no new kit-side files added; this gate is unaffected).
- [x] AC-X6: `python scripts/check_invariant_ids.py` passes.
- [x] AC-X7: No source / aspect-manifest / protocol-prose / CI / new-spec / new-design / new-ADR / new-principle changes.
