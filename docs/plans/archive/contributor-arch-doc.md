...
- [x] `docs/contributing.md` exists with the six sections above, ≤ 250 lines.
- [x] Module map table covers every file currently in `src/kanon/*.py` and `src/kanon/_validators/*.py`.
- [x] Gate matrix covers every script in `scripts/check_*.py` and every workflow in `.github/workflows/*.yml`.
- [x] Cross-links added to `README.md` and `AGENTS.md`; both link forms render correctly.
- [x] No emoji unless explicitly requested by the user.
- [x] No CHANGELOG entry needed (docs-only — per `AGENTS.md § Contribution Conventions`).
- [x] `kanon verify .` returns `status: ok` after the change.
- [x] No existing markdown link is broken (`scripts/check_links.py` passes).
