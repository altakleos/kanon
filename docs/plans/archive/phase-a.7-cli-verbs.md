...
- [x] AC-M1: `src/kanon/cli.py` exposes `kanon resolutions check`, `kanon resolutions explain`, `kanon contracts validate`, `kanon resolve` verbs
- [x] AC-M2: `kanon resolutions check` returns `status: ok` for clean / missing resolution; `status: fail` exit 1 for stale
- [x] AC-M3: `kanon contracts validate <missing>` errors; `kanon contracts validate <valid-empty-bundle>` returns ok
- [x] AC-M4: `kanon resolve` and `kanon resolutions explain` exit 0 with stub messaging
- [x] AC-T1: ≥10 tests passing across two new test files
