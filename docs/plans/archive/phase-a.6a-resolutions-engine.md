...
- [x] AC-M1: `src/kanon/_resolutions.py` exists with at minimum: `replay`, `stale_check`, `canonicalize_entry`, `ReplayReport`, `ReplayError`, `ExecutionRecord` symbols.
- [x] AC-M2: `replay(target=Path('/nonexistent'))` returns an empty `ReplayReport` (no `.kanon/resolutions.yaml` = no replay; not an error).
- [x] AC-M3: `canonicalize_entry({"meta-checksum": "x", "b": 1, "a": [3, 2]})` returns deterministic bytes; `meta-checksum` field stripped.

### Tests

- [x] AC-T1: `tests/test_resolutions.py` exercises hand-edit detection (3 cases).
- [x] AC-T2: Quadruple-pin checks (≥8 cases — positive + drift per pin).
- [x] AC-T3: Evidence grounding (≥2 cases — non-empty passes; empty fails).
- [x] AC-T4: Replay determinism (2 runs of same input produce identical reports).
- [x] AC-T5: Stale detection (any pin drift surfaces an error; replay continues).
- [x] AC-T6: Full-cycle integration (build synthetic contract + resolution, assert clean replay).
- [x] AC-T7: All tests pass.

### CHANGELOG

- [x] AC-X1: `CHANGELOG.md` `[Unreleased] § Added` gains a paragraph naming Phase A.6a.
...
- [x] AC-X8: No `src/kanon/cli.py` change (CLI verbs are A.7).
- [x] AC-X9: No `src/kanon/_verify.py` change (integration deferred).
- [x] AC-X10: No `src/kanon_reference/` change.
- [x] AC-X11: Full pytest passes (no regression beyond the new module's tests).
