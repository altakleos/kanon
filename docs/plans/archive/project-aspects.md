...
- [x] T10: Update CI `check_kit_consistency.py` — assert all kit-side aspect names match `^kanon-[a-z][a-z0-9-]*$`; fold the existing `_check_cross_aspect_exclusivity` into the new namespaced-paths shape → `scripts/check_kit_consistency.py`. (depends: T6)
...
- [x] T13: Tests: `check_kit_consistency.py` rejects a kit-side directory named `project-foo`; rejects a top-manifest entry without the `kanon-` prefix → `tests/scripts/test_check_kit_consistency.py`. (depends: T10)
