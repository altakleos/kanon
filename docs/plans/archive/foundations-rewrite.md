...
- [x] AC-V1: `vision.md` opens with "kanon is a portable, self-hosting protocol substrate for prose-as-code engineering discipline."
- [x] AC-V2: Vision contains a **Public protocol commitments** section listing exactly the six public principles by ID with one-line statements.
- [x] AC-V3: Vision retires tier vocabulary throughout; uses depth/aspect/recipe/dialect/publisher consistently.
- [x] AC-V4: Vision contains an **Amendment trail** preserving existing entries (ADR-0013, ADR-0015) and adding ADR-0048.
- [x] AC-V5: Vision contains a **Historical Note** preserving the v0.1 kit-shape vision body verbatim with predecessor commit SHA.

### Principles

- [x] AC-P1: Every existing principle file gains a `tier: public-protocol | kit-author-internal` frontmatter field.
- [x] AC-P2: Three new principle files exist: `P-protocol-not-product.md`, `P-publisher-symmetry.md`, `P-runtime-non-interception.md`. All `tier: public-protocol`. All carry `status: accepted`.
- [x] AC-P3: `P-tiers-insulate.md` carries `status: superseded` + `superseded-by: 0048`; body preserved verbatim.
- [x] AC-P4: `P-specs-are-source.md`, `P-self-hosted-bootstrap.md`, `P-verification-co-authored.md` each carry a `## Historical Note` section preserving their pre-amendment body with predecessor commit SHA.
- [x] AC-P5: `docs/foundations/principles/README.md` index reorganized: 6 public-protocol entries first, 2 kit-author-internal second, 1 superseded section. Preamble explains dialect-citation discipline.

### Personas

- [x] AC-Per1: `solo-with-agents.md` and `onboarding-agent.md` each carry an `## Amendments` section with the protocol-shape vocabulary refresh.
- [x] AC-Per2: `solo-engineer.md` and `platform-team.md` carry `status: superseded` + brief rationale; bodies preserved.
- [x] AC-Per3: `acme-publisher.md` exists with `status: accepted`, follows the existing persona template (one-sentence summary, Context, Goals, What stresses the kit, What does NOT stress, Success).
- [x] AC-Per4: `docs/foundations/personas/README.md` index updated with active/superseded split.

### Manifesto

- [x] AC-M1: `docs/foundations/de-opinionation.md` exists with the six sections specified above.
- [x] AC-M2: Manifesto cites vision.md and ADR-0048; is cited from vision.md.

### ADR

- [x] AC-A1: `docs/decisions/0048-kanon-as-protocol-substrate.md` exists with `status: accepted`, follows ADR template, contains: Context, Decision, Alternatives Considered, Consequences (including the public/internal principle tiering and the supersession of ADR-0012 in part).
- [x] AC-A2: `docs/decisions/README.md` index updated with ADR-0048.
...
- [x] AC-X4: No source / spec / aspect-manifest / protocol-prose changes.
- [x] AC-X5: CHANGELOG.md gains an `## [Unreleased]` entry summarizing the foundations rewrite (this IS user-visible — substrate identity changes).
- [x] AC-X6: Doc length: vision ≤ 300 lines; manifesto ≤ 200 lines; each new principle ≤ 100 lines; new persona ≤ 100 lines.
