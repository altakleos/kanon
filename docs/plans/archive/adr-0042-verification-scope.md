...
- [x] AC-A1: `docs/decisions/0042-verification-scope-of-exit-zero.md` exists with `status: accepted` and the six required ADR sections.
- [x] AC-A2: ADR-0042 cites ADR-0048, ADR-0039, ADR-0041, the verification-contract spec, and INV-11.
- [x] AC-A3: At least four Alternatives Considered.
- [x] AC-A4: Decision section includes the canonical exit-zero wording verbatim — short, citable, copy-pasteable.

### Spec amendment

- [x] AC-S1: `docs/specs/verification-contract.md` gains a cross-reference paragraph pointing at ADR-0042; INV-11's body unchanged.

### Indexes + CHANGELOG

- [x] AC-X1: `docs/decisions/README.md` updated with ADR-0042 row.
- [x] AC-X2: `CHANGELOG.md` `[Unreleased] §Added` gains a paragraph naming ADR-0042.

### Cross-cutting

- [x] AC-X3: `kanon verify .` returns `status: ok`, zero warnings.
- [x] AC-X4: `python scripts/check_links.py` passes.
- [x] AC-X5: `python scripts/check_foundations.py` passes, zero warnings.
- [x] AC-X6: `python scripts/check_invariant_ids.py` passes, zero warnings.
- [x] AC-X7: No source / aspect-manifest / protocol-prose / CI / spec-body changes (only verification-contract.md amendment is a cross-reference, no INV body changes; only NEW spec content is none).
