...
- [x] AC-M1: `src/kanon/_realization_shape.py` exists with `V1_DIALECT_VERBS`, `RealizationShape`, `ShapeValidationError`, `parse_realization_shape`, `validate_resolution_against_shape` symbols.
- [x] AC-M2: `V1_DIALECT_VERBS` is a frozenset containing exactly the 9 canonical verbs (`lint`, `test`, `typecheck`, `format`, `scan`, `audit`, `sign`, `publish`, `report`).
- [x] AC-M3: `parse_realization_shape({"verbs": ["lint"], "evidence-kinds": ["config-file"], "stages": []}, dialect="2026-05-01")` returns a `RealizationShape` instance.
- [x] AC-M4: Parser raises `click.ClickException` for missing required keys, non-list verbs, unknown verbs.
- [x] AC-M5: Validator returns `[]` for a clean resolution; structured `ShapeValidationError` list otherwise.

### Tests

- [x] AC-T1: `tests/test_realization_shape.py` exercises ≥15 cases.
- [x] AC-T2: Full pytest passes.

### CHANGELOG

- [x] AC-X1: `CHANGELOG.md` `[Unreleased] § Added` gains a paragraph naming Phase A.6c.
...
- [x] AC-X9: No `_resolutions.py` integration.
- [x] AC-X10: No CLI verb additions.
