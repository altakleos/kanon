...
- [x] AC-S1: `packaging/substrate/pyproject.toml` exists; parses as TOML.
- [x] AC-S2: `[project].name = "kanon-core"`; `version = "1.0.0a1"`; `requires-python = ">=3.10"`; `dependencies` include `click>=8.1` + `pyyaml>=6.0`.
- [x] AC-S3: `[project.scripts] kanon = "kanon.cli:main"` present.
- [x] AC-S4: `[tool.hatch.build.targets.wheel].exclude` contains `"../../src/kanon/kit/aspects/**"`.

### Reference pyproject

- [x] AC-R1: `packaging/reference/pyproject.toml` exists; parses as TOML.
- [x] AC-R2: `[project].name = "kanon-aspects"`; `version = "1.0.0a1"`; depends on `kanon-core==1.0.0a1`.
- [x] AC-R3: `[project.entry-points."kanon.aspects"]` exists as a *commented-out* stub for traceability (one comment line per the seven aspects); not active.

### Kit meta pyproject

- [x] AC-K1: `packaging/kit/pyproject.toml` exists; parses as TOML.
- [x] AC-K2: `[project].name = "kanon-kit"`; `version = "1.0.0a1"`; depends on both `kanon-core==1.0.0a1` and `kanon-aspects==1.0.0a1`.
- [x] AC-K3: Sentinel module `packaging/kit/src/_kanon_kit_meta/__init__.py` exists with a one-line docstring.

### CI gate

- [x] AC-CI1: `scripts/check_packaging_split.py` exists; runnable; returns `{"status":"ok"}` JSON on green path.
- [x] AC-CI2: `tests/scripts/test_check_packaging_split.py` covers green path + each invariant failure (≥6 tests).
- [x] AC-CI3: Test file follows the existing CI-test conftest shape (uses `load_ci_script` fixture).

### Cross-cutting

- [x] AC-X1: `CHANGELOG.md` `[Unreleased] § Added` gains a paragraph naming Phase A.1.
...
- [x] AC-X7: `python scripts/check_packaging_split.py` passes (the new gate validates itself green).
- [x] AC-X8: `uv run --frozen pytest -q --no-cov` passes (no regression).
- [x] AC-X9: Top-level `pyproject.toml` is byte-identical to current `main` after this PR.
- [x] AC-X10: No `src/kanon/` content changed; no aspect content moved; no `_kit_root()` change; no source code change in any kanon module.
