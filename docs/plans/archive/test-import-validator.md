...
Add a kit validator that detects test files under `tests/scripts/` referencing
...
- Scans `tests/scripts/test_*.py` for the canonical path-construction pattern
...
- Test files outside `tests/scripts/` (those don't follow the CI-script
  companion pattern).
...
- [x] Add tests: orphan detected, clean repo passes, missing tests/scripts/
...
3. The validator skips projects without a `tests/scripts/` directory.
