...
- [x] AC-Y1: `src/kanon/kit/manifest.yaml` no longer contains `defaults:` block.
- [x] AC-Y2: `src/kanon/kit/manifest.yaml` no longer contains `files:` block.
- [x] AC-Y3: `src/kanon/kit/manifest.yaml` `aspects:` block remains (dead but preserved).

### Substrate

- [x] AC-S1: No source file references `top.get("defaults"`, `top["defaults"]`, or equivalent.
- [x] AC-S2: No source file references `top.get("files"`, `top["files"]`, kit-global `files:`, or `_kit_root() / "files"`.
- [x] AC-S3: No source file references `kit.md` for read/write/render.
- [x] AC-S4: `src/kanon/kit/kit.md` template file no longer exists.
- [x] AC-S5: `.kanon/kit.md` (kanon repo's consumer-side artifact) no longer exists.
- [x] AC-S6: `_scaffold.py:_render_kit_md()` deleted.

### Gate

- [x] AC-G1: `scripts/check_kit_consistency.py:_check_kit_md_exists()` deleted; not called from `run_checks()`.
- [x] AC-G2: `tests/scripts/test_check_kit_consistency.py::test_missing_kit_md_detected` and `::test_kit_md_bad_heading_detected` deleted.

### Fidelity

- [x] AC-F1: `.kanon/fidelity.lock` regenerated; `kanon verify .` returns `status: ok`, zero warnings.

### Tests

- [x] AC-T1: All test files audited; kit.md / defaults: / files: references removed or updated.
- [x] AC-T2: Full pytest suite passes (no regression beyond removed obsolete tests).

### CHANGELOG

- [x] AC-X1: `CHANGELOG.md` `[Unreleased] § Removed` gains a paragraph naming Phase A.3.
...
- [x] AC-X5: `python scripts/check_kit_consistency.py` passes (after `_check_kit_md_exists` deletion).
- [x] AC-X6: `python scripts/check_invariant_ids.py` passes.
- [x] AC-X7: `python scripts/check_packaging_split.py` passes.
- [x] AC-X8: No `src/kanon_reference/` change (out of scope).
- [x] AC-X9: No `src/kanon/kit/aspects/` content moved (out of scope; defer to content-move sub-plan).
