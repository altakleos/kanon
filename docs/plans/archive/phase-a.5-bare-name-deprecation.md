...
- [x] AC-S1: `_normalise_aspect_name(bare_name)` emits a stderr warning containing "deprecated" and the full `kanon-<X>` name.
- [x] AC-S2: `_normalise_aspect_name(namespaced_name)` (e.g., `kanon-sdd`, `project-foo`) emits no warning.
- [x] AC-S3: Function still returns the correct sugared name (behaviour preserved).
- [x] AC-S4: Function docstring updated to mark sugar as deprecated.

### Tests

- [x] AC-T1: New `tests/test_bare_name_deprecation.py` covers the 3 scenarios above.
- [x] AC-T2: Full pytest passes (existing tests' stderr assertions, if any, accommodate the new warning).

### CHANGELOG

- [x] AC-X1: `CHANGELOG.md` `[Unreleased] § Deprecated` gains a paragraph naming Phase A.5.
...
- [x] AC-X9: No aspect content moved.
- [x] AC-X10: `_normalise_aspect_name`'s sugar path still functions (only warns; doesn't fail).
