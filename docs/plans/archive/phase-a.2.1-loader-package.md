...
- [x] AC-P1: `src/kanon_reference/__init__.py` exists with a one-line docstring.
- [x] AC-P2: `src/kanon_reference/aspects/__init__.py` exists with a one-line docstring.
- [x] AC-P3: Seven modules `src/kanon_reference/aspects/kanon_{deps,fidelity,release,sdd,security,testing,worktrees}.py` exist, each with a top-level `MANIFEST: dict[str, Any]`.
- [x] AC-P4: For each of the seven aspects, the LOADER MANIFEST is semantically equivalent to `yaml.safe_load(src/kanon/kit/aspects/<X>/manifest.yaml)`.

### Pyproject update

- [x] AC-Y1: `packaging/reference/pyproject.toml` has an active (uncommented) `[project.entry-points."kanon.aspects"]` block.
- [x] AC-Y2: Each of the seven entries points at `kanon_reference.aspects.kanon_<id>:MANIFEST`.
- [x] AC-Y3: `[tool.hatch.build.targets.wheel].packages` references `../../src/kanon_reference`.
- [x] AC-Y4: The placeholder file `packaging/reference/src/_kanon_reference_placeholder/__init__.py` and its parent dirs are deleted.

### CI gate

- [x] AC-CI1: `scripts/check_packaging_split.py` gains a `_check_reference_entry_points` function that validates the seven entry-points are present and target `kanon_reference.aspects.kanon_<id>:MANIFEST`.
- [x] AC-CI2: `tests/scripts/test_check_packaging_split.py` covers green path + missing-entry-points failure + wrong-entry-point-target failure.
- [x] AC-CI3: New test `tests/test_kanon_reference_manifests.py` parametrized over the seven aspects asserts MANIFEST ≡ YAML (semantic equivalence).

### Cross-cutting

- [x] AC-X1: `CHANGELOG.md` `[Unreleased] § Added` gains a paragraph naming Phase A.2.1.
- [x] AC-X2: `kanon verify .` returns `status: ok`, zero warnings.
- [x] AC-X3: `python scripts/check_links.py` passes.
- [x] AC-X4: `python scripts/check_foundations.py` passes.
- [x] AC-X5: `python scripts/check_kit_consistency.py` passes.
- [x] AC-X6: `python scripts/check_invariant_ids.py` passes.
- [x] AC-X7: `python scripts/check_packaging_split.py` passes.
- [x] AC-X8: `pytest tests/test_kanon_reference_manifests.py --no-cov` → 7 passed.
- [x] AC-X9: Full pytest: ≥841 passed (was 834 + 7 new manifest-equivalence + new gate-failure tests), 0 failed.
- [x] AC-X10: No `src/kanon/` content changed (kernel and YAML manifests untouched). No aspect content moved. No `_kit_root()` change.
- [x] AC-X11: Top-level `pyproject.toml` byte-identical to current `main`.
