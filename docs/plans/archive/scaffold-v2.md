...
- [x] P1-T1: Add `files:` key to top-level manifest.yaml for kit-global
  files. Move `.kanon/kit.md` there. Create `src/kanon/kit/files/`
  directory.
- [x] P1-T2: Add `files:` key support to each aspect sub-manifest
  (aspect-level files). Initially empty for all aspects.
- [x] P1-T3: Update `_aspect_files()` in `_manifest.py` to prepend
  aspect-level `files:` before the depth union.
- [x] P1-T4: Update `_build_bundle()` in `_scaffold.py` to read
  kit-global files first.
- [x] P1-T5: Update `_expected_files()` to include kit-global and
  aspect-level files.
- [x] P1-T6: Rewrite `kit.md` template to be aspect-neutral (no
  sdd-specific references).
- [x] P1-T7: Tests for kit-global and aspect-level file resolution.

## Phase 2: Content moves (INV-5, INV-6, INV-7)

Move inlined AGENTS.md content to protocol files. Eliminate sections.

- [x] P2-T1: Create protocol files for content that only exists as
...
- [x] P2-T2: Delete all `sections/` directories from kit aspects.
  Remove `sections:` keys from all sub-manifests.
- [x] P2-T3: Delete all `agents-md/` body files from kit aspects.
- [x] P2-T4: Rename `docs/sdd-method.md` →
...
- [x] P2-T5: Update all cross-references to sdd-method.md
  (~30 living documents; leave ADR references as historical).
- [x] P2-T6: Update byte-equality entries in sdd sub-manifest.
- [x] P2-T7: Mirror new protocol files to kit aspect protocol dirs.

## Phase 3: AGENTS.md routing index (INV-3, INV-4, INV-9)

Rewrite AGENTS.md assembly to produce the slim routing index.

- [x] P3-T1: Rewrite `agents-md-base.md` template to the ~80-line
...
- [x] P3-T2: Simplify `_assemble_agents_md()` — remove body injection,
  section filling, inactive-section cleanup. Keep only: load base +
  render protocols-index.
- [x] P3-T3: Update `_render_protocols_index()` to include former
...
- [x] P4-T1: Remove CLAUDE.md from sdd depth-0 files (it's a harness
  shim, handled by harnesses.yaml).
- [x] P4-T2: Change worktrees `requires: "kanon-sdd >= 1"` to
  `suggests: "kanon-sdd >= 1"`. Audit other aspects for sdd
  hard-dependencies.
- [x] P4-T3: Update `_default_aspects()` — move the sdd:1 default
  to CLI layer (init command default), not manifest truth.
- [x] P4-T4: Handle zero-aspect edge case in `kanon verify` (warn,
  don't error).
- [x] P4-T5: Test: `kanon init --aspects worktrees:1,testing:1`
  produces valid project with no sdd files.
- [x] P4-T6: Test: `kanon init --aspects ""` (bare) produces minimal
  valid project.

## Phase 5: ADR + cleanup

- [x] P5-T1: Write superseding ADR for ADR-0010 (refines
  enforcement-proximity: hard gates inline as compressed table,
  soft guidance protocol-only).
- [x] P5-T2: Update `docs/specs/README.md` with scaffold-v2 entry.
- [x] P5-T3: Update `docs/plans/README.md` with this plan.
- [x] P5-T4: Update `CHANGELOG.md` with all changes.
- [x] P5-T5: Regenerate `fidelity.lock`.
- [x] P5-T6: Run full test suite + `kanon verify .` + all CI scripts.
