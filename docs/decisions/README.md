...
| [0052](0052-aspects-flatten.md) | Aspects-flatten path selection — defers `src/kanon_reference/` → `aspects/` (same Hatch editable-install constraint as ADR-0050; Option C accepted-debt over Option B's 6–8h rename cost) | superseded by ADR-0054 | process |
...
| [0054](0054-final-layout-and-core-vocabulary.md) | Final layout for the deferred multi-distribution split + "core" vocabulary — `packages/kanon-{core,aspects,kit}/src/<pkg>/` per panel synthesis (5 of 6 second-choice votes); Hatchling prefix-REMOVE `sources = ["src"]` form sidesteps `editables` wall; "core" replaces "kernel" prospectively; supersedes ADR-0052; supersedes ADR-0050 §Option A in part | accepted | process |
| [0060](0060-foundations-review-and-impact.md) | Foundations review protocol and downstream impact validator — structured review when vision changes; mechanical `realizes:`/`stressed_by:` tracing | accepted | process |
