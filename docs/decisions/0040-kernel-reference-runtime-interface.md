...
§Decision item 7 (`_kit_root()` retirement) is operationally complete in two stages:

- **Runtime stage** — landed on main as commits `4b243ff` (replace runtime `_kit_root()` calls with `_kit_data()` per Phase A.2) and `6dd8628` (delete the dead `_kit_root()` function and add `_kit_data` tests).
- **CI-gate stage** — landed in the `wt/exec-retire-kit-aspects-yaml` PR alongside this note, per the panel-ratified plan at [`docs/plans/archive/retire-kit-aspects-yaml.md`](../plans/archive/retire-kit-aspects-yaml.md). T4 deletes `packages/kanon-core/src/kanon_core/kit/manifest.yaml` (the documentation-layer residue of the retired `_kit_root()` model); T1, T2, T3 reroute `scripts/check_kit_consistency.py`, `scripts/check_package_contents.py`, and the drift sentinel in `tests/test_aspect_provides.py` to read `pyproject.toml`'s `[project.entry-points."kanon.aspects"]` table + each aspect's per-aspect `manifest.yaml` (canonical per ADR-0055) instead.

Both stages preserve the §Decision §1 entry-points discovery contract; this note records that the retirement crossed every code-path layer the original ADR named. Per ADR-0032 exception class 2, this section is a strict-suffix `## Historical Note` append; the body of §Decision and the alternatives above are unchanged.
