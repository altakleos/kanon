...
`kanon` needs to serve projects across a wide complexity range: from a 200-line CLI utility to a multi-team platform project. A single uniform "full SDD" method would overwhelm the former; a single uniform "minimal SDD" would underserve the latter. The kit adopts a tiered model.
...
| Tier | For | Adds |
|---|---|---|
| 0 | Vibe-coding, prototypes, short-lived scripts | `AGENTS.md` + `CLAUDE.md` shim + `.kanon/config.yaml`. No `docs/` structure. |
...
Tier is stored in `.kanon/config.yaml` (field: `tier`). The CLI reads this field to determine expected layout and which AGENTS.md sections to enable. Tier is **not** inferred from filesystem contents (that would be ambiguous when a user deletes a directory).
...
- Every consumer repo declares its tier explicitly in `.kanon/config.yaml`.
- `kanon verify` computes the expected file set as a function of declared tier and hard-fails if expected files are missing.
...
```yaml
kit_version: "0.1.0a1"
tier: 1                  # 0 | 1 | 2 | 3 — mutable via `kanon tier set`
```
