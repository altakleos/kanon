...
1. **Four tiers ship in v0.1.** Each tier's bundle is a named directory under `src/kanon/templates/`:
   - `tier-0/` — minimal: `AGENTS.md`, `CLAUDE.md`, `.kanon/config.yaml`. No `docs/` structure.
...
5. **HTML-comment markers.** Every AGENTS.md template has its kit-managed sections wrapped in `<!-- kanon:begin:<section> -->` / `<!-- kanon:end:<section> -->` comment pairs. The section list for each tier is defined in `tier-migration.md`.
6. **`.kanon/config.yaml` seed.** Every tier writes this file with `{kit_version: "<installed-version>", tier: <N>, tier_set_at: "<ISO-8601>"}`. `init` errors out if `.kanon/` already exists and `--force` wasn't given.
7. **No consumer state leaks into the wheel.** `ci/check_package_contents.py` hard-fails on any file under paths that look like consumer output (e.g., `.kanon/`, `.kit/fidelity.lock`, test-only artifacts).
