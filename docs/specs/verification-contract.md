...
stressed_by:
  - platform-team
fixtures_deferred: "`agent-sdd verify` integration tests land in Phase D/F"
---
# Spec: Verification contract — what `agent-sdd verify` guarantees

## Intent

Define the checks `agent-sdd verify <target>` runs on a consumer repo, the error/warning taxonomy it emits, and the guarantees it provides to the consumer.

## Invariants

1. **Tier-aware.** `verify` reads `.agent-sdd/config.yaml` → `tier: <N>` and computes the expected file set as a function of N. Absence of `config.yaml` is a hard error.
...
5. **AGENTS.md marker integrity.** `verify` checks that every `<!-- agent-sdd:begin:X -->` has a matching `<!-- agent-sdd:end:X -->` and that the set of enabled sections matches the declared tier. Mismatches are hard errors.
