...
| `aspect` | `src/kanon/kit/manifest.yaml` aspects key + `src/kanon_reference/aspects/kanon_<name>/` directory | `worktrees` | `requires:` depth-predicates, `provides:` capability owner aspect, AGENTS.md `<!-- kanon:begin:<aspect>/* -->` markers, consumer `.kanon/config.yaml` `aspects.<name>:` |
...
4. **Postcondition: the kit's CI fleet passes.** After a successful rename, the following all return exit 0 against the renamed repo: `python scripts/check_foundations.py`, `python scripts/check_invariant_ids.py`, `python scripts/check_verified_by.py`, `python scripts/check_links.py`, `python scripts/check_kit_consistency.py`, `kanon verify .`. The tool runs this fleet as a self-check before clearing its sentinel; if any check fails, the sentinel persists and the operation reports which check rejected the rename.

<!-- INV-spec-graph-rename-byte-equality -->
5. **Byte-equality boundary handling.** Files in the kit-bundle byte-equality whitelist (declared per-aspect in `src/kanon_reference/aspects/kanon_<name>/manifest.yaml` under `byte-equality:` per ADR-0011) have a kit-side mirror at `src/kanon_reference/aspects/kanon_<name>/files/<rel>`. When the rename touches a repo-canonical file with a kit mirror, both copies are rewritten together in the same ops-manifest run; INV-3's atomicity covers them as a single transaction.
