...
Make `kanon`-managed repos safe for multiple concurrent LLM agents — parallel worktrees, orchestrators spawning sub-agents, long-running background jobs. The kit's artifacts become the inter-agent protocol, not just human documentation.
...
5. **Sub-agent AGENTS.md inheritance.** `kanon agents-preamble` CLI emits the resolved AGENTS.md boot chain as a single blob; orchestrators prepend this to every sub-agent system prompt.
