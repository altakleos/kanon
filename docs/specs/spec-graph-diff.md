...
5. **Renamed anchors are detected when source SHAs match, otherwise reported as remove+add.** If an anchor present in `<old-rev>` has a different name in `<new-rev>` but the body content matches byte-for-byte, the diff renders this as `renamed: <old-anchor> → <new-anchor>`. If the body also changed, the diff renders as `removed: <old-anchor>` + `added: <new-anchor>` — the tool does not heuristically guess at simultaneous body+name edits. (Anchor renames produced by `kanon graph rename --type inv-anchor` are deterministic and would have a body match; manual edits are not.)
