...
**Early alpha (v0.3.1a1).** The kit ships seven aspects (`sdd` at depth 0–3; `worktrees`, `release`, `security`, `deps` at depth 0–2; `testing` at depth 0–3; `fidelity` at depth 0–1), the aspect model with a `provides:` capability registry, cross-harness shims, and self-hosting assertions. See [the roadmap](docs/plans/roadmap.md) for what's coming.
