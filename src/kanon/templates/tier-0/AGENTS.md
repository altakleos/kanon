...
A tier-0 `kanon` project. The kit is installed but no process gates are active yet — ideal for vibe-coding, prototypes, or short-lived utilities.

## Boot chain

Read this file. That's it for tier-0. There are no further artifacts required.

## What `kanon` gives you at tier-0

- A pointer file (`AGENTS.md`) that every supported LLM agent harness will read (via shims).
- Consistent `.kanon/config.yaml` bookkeeping (tier, kit version).
